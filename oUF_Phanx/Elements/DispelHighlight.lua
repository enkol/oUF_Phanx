...
--				local name, _, _, _, type = UnitDebuff(unit, i)
				...
				local aura = C_UnitAuras.GetBuffDataByIndex(unit, i, "HARMFUL")
				if not aura then break end
				-- print("UnitDebuff", unit, i, tostring(name), tostring(type))
				if aura.dispelName and (not debuffType or (ClassDispelPriority[aura.dispelName] and ClassDispelPriority[aura.dispelName] > ClassDispelPriority[debuffType])) then
					-- print("debuffType", type)
					debuffType = aura.dispelName
					dispellable = canDispel[aura.dispelName]
				end
			end
		end
	elseif UnitCanAttack("player", unit) then
		if canSteal or next(canPurge) then
			for i = 1, 40 do
				...
				if not aura then break end
				if (canSteal and aura.isStealable) or (aura.dispelName and canPurge[aura.dispelName]) then
					-- print("debuffType", aura.dispelName)
					debuffType = aura.dispelName
					dispellable = true
					break
				end
